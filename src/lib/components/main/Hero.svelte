...
>
  ...
  <div class="absolute inset-0 overflow-hidden pointer-events-none z-0">
    {#each fallingLogos as logo (logo.id)}
      <div
        ...
        "
      >
        <Logo width="100%" height="100%" className="color: #fff;" />
      </div>
    {/each}
  </div>
  ...
  >
    ...
    >
      ...
      <!-- Cubic Logo -->
      <div
        class="relative transform group-hover:scale-105 transition-all duration-700 w-full h-full"
      >
        <Logo width="100%" height="100%" className="color #fff;" />
      </div>
    </div>
    ...
    <!-- Text & Buttons -->
    <h1
      class="text-6xl md:text-8xl font-bold leading-[0.9] tracking-tighter mb-8 bg-gradient-to-b from-white to-neutral-400 bg-clip-text text-transparent"
    >
