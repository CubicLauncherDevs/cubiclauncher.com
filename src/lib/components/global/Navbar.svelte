  ...
  <div class="pointer-events-auto w-full px-4 sm:px-6 flex justify-center">
    <div
      class="w-full flex items-center justify-between transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] rounded-full border {scrolled
        ? 'max-w-4xl bg-neutral-900/70 backdrop-blur-xl border-white/10 px-4 sm:px-8 py-2 sm:py-3 shadow-2xl'
    ...
    >
      <div class="flex items-center gap-4 sm:gap-12 shrink-0">
        <a href="/" class="flex items-center gap-3 group">
          ...
          />
          <span class="hidden sm:block font-bold text-lg tracking-tighter uppercase"
            >CubicLauncher</span
          >
        </a>
...
{#if isMobileMenuOpen}
  ...
  >
    ...
    <nav class="flex flex-col items-center gap-10">
      <a href="/" class="mb-8 flex items-center gap-3" on:click={toggleMobileMenu}>
        <Logo width="3rem" height="3rem" color="#ffffff" />
        <span class="font-bold text-2xl tracking-tighter uppercase text-white">CubicLauncher</span>
      </a>
